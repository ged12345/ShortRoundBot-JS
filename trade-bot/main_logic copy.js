...
class MainLogic {
    ...
    constructor(kraken, exchangeName) {
        ...
        this.unprofitableCoins = {
            ...
            avoidUnprofitableCoins: false,
            unprofitableCoinsAllowedCount: 0,
            unprofitableCoinsAllowedMax: 3,
            checkCoinId: (coinId) => {
                let foundCoinId = this.unprofitableCoins.coinIdArray.filter((el) => {
    ...
    async setupQueues() {
        ...
        /*API.addTradeRecord(this.id, {
            timestamp: "1234687875",
            tradeId: "TX5687", 
            tradeType: "BUY", 
            tradeVolume: 100, 
            tradePrice: 30567, 
            coinExchangeId: "USDBTC", 
            profitLoss: 0.45
        }, () => {
        });*/
        //process.exit(0);
    ...
    checkAdvice() {
        ...
        /* Do we do this or just rely on bringing up the bottom trade? */
        if (
            (Number(coinAdvice['probability']) >= 75
