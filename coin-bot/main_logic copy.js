...
const {
    standardiseOHLCResultBybit,
} = require('../utils/standardiseApiResults.js');
...
class MainLogic {
    ...
    async setupQueues() {
        ...
        let numberOfCoins = 5; /* For testing purposes */
        ...
        /* We up this for each trend/signal we calculate, and for each coin - why? */
    ...
    async setupOHLCAcquisitionQueue() {
        ...
        this.coinConfigArr.forEach((coin) => {
            this.OHLCAcquisitionQueue.enqueue(async () => {
                ...
                if (coin[`coin_id_${this.exchange.name}`] !== '') {
                    this.getOHLC(
                        coin['id'],
        ...
        this.exchange.curr.OHLC(coinPair, 1, async (result) => {
            ...
            try {
                ...
                for (const ohlcEl of ohlcDesc) {
                    await this.mysqlCon.storeCoinOHLC(
                        coinId,
                        standardiseOHLCResultBybit(ohlcEl)
                    );
    ...
    getOHLCArray(ohlcResult, coinPair) {
        let ohlcDesc = null;
        let ohlcExchangeResult = null;

        /* Different exchanges parse information differently */
        if (this.exchange.name === 'bybit') {
            ohlcExchangeResult = ohlcResult['result'];
        } else {
            ohlcExchangeResult = ohlcResult[coinPair];
        }

        if (this.simulateTimestamp === -1) {
            ohlcDesc = ohlcExchangeResult.reverse();
        } else {
            // For timestamp simulation
            ohlcDesc = ohlcExchangeResult.reverse().filter((el) => {
        ...
        this.coinConfigArr.forEach((coin) => {
            ...
            this.PlotlyGraphingQueue.enqueue(async () => {
                ...
                if (coin[`coin_id_${this.exchange.name}`] !== '') {
                    ...
                    let resultsEMA = await this.mysqlCon.getProcessedEMA(
                        coinId
                    );
