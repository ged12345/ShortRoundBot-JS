class BinanceExchange {
    ...
    OHLC(coinPair, interval, cb) {
        this.api.candlesticks(coinPair, `${interval}m`, (error, ticks, symbol) => {
    ...
    addOrder(orderDetails, cb) {
        ...
        if (orderDetails.ordertype === "stop-loss") {
            ...
            if(orderDetails.type === "buy") {
                this.api.marketBuy(orderDetails.pair, orderDetails.volume, (error, response) => {
                    cb(response);
                });
            } else if(orderDetails.type === "sell") {
                this.api.marketSell(orderDetails.pair, orderDetails.volume, (error, response) => {
                    cb(response);
                });
            }
        ...
        this.api.sell(orderDetails.pair, orderDetails.volume, orderDetails.price, {stopPrice: orderDetails.price, type: orderType}, (error, response) => {
    ...
    queryOrders(coinPair, txid, cb) {
        this.api.orderStatus(coinPair, txid, (error, orderStatus, symbol) => {
            ...
            orderQuery[coinPair]["status"] = Number(orderStatus["price"]) * Number(orderStatus["executedQty"]);
